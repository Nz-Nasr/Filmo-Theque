...
// شروع حلقه وردپرس برای گرفتن اطلاعات پست (فیلم)
if (have_posts()) : while (have_posts()) : the_post();

        $post_id = get_the_ID();

        $featured = has_post_thumbnail($post_id) ? get_the_post_thumbnail_url($post_id, 'full') : get_template_directory_uri() . '/assets/images/films/default.jpg';
        ...
        <section class="relative w-full max-w-[1440px] mx-auto my-16 py-8 px-4 sm:px-6">
            ...
            <div class="relative z-10 grid lg:grid-cols-12 gap-8 items-start">
                ...
                <div class="lg:col-span-4 flex flex-col items-center mt-6 lg:mt-12 mb-8 lg:mb-20">
                    ...
                    <!-- عنوان فیلم برای موبایل (تو دسکتاپ مخفیه) -->
                    ...
                    <?php if ($imdb):
                        ...
                        // true: فقط یک مقدار ساده برگردون، نه آرایه
                    ?>
                        ...
                        <div class="text-[#0D1B2A] text-center max-w-xs">
                            ...
                            <?php if ($imdb_url): ?>
                                <!-- نمایش لوگوی IMDb، با لینک به سایت IMDb اگر آدرس موجود باشه -->
                            ...
                            <?php else: ?>
                                <!-- اگه لینک  نبود، فقط لوگو رو نشون می‌دیم -->
                                <img src="<?php echo get_template_directory_uri(); ?>/assets/images/logo/IMDB.png"
                                    class="w-16 h-auto mx-auto mt-2">
                            <?php endif; ?>
...
<section class="w-full max-w-[1440px] mx-auto my-16 px-4 sm:px-6">
    <h2 class="text-[#0D1B2A] text-2xl font-bold mb-6 text-right">ثبت نظر</h2>
...
<section class="w-full max-w-[1440px] mx-auto my-16 px-4 sm:px-6">
    <h2 class="text-[#0D1B2A] text-2xl font-bold mb-6 text-right">نظرات کاربران</h2>

    <div class="space-y-6">
        <?php
        // اگه فرم نظر پر شده بود، نظر جدید رو ثبت می‌کنیم
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['comment_name']) && !empty($_POST['comment_email']) && !empty($_POST['comment_content'])) {
            $new_comment = array(
                ...
                'comment_approved' => 1, // کامنت فوری تایید بشه
            );
            wp_insert_comment($new_comment);
            // نظر رو تو دیتابیس وردپرس ذخیره می‌کنیم
            echo '<p class="text-green-600 font-semibold">نظر شما با موفقیت ثبت شد.</p>';
        }

        // نظرات تأییدشده رو برای این پست می‌گیریم
        $comments = get_comments(array(
            'post_id' => $post_id,
            'status' => 'approve'
        ));

        // اگه نظراتی وجود داشت، نمایششون می‌دیم
        if ($comments) :
